 ...
 * [서버 조립 파일]
 *
 * 들어오는 요청을 어떤 기능으로 보낼지 정하는 교통정리 담당입니다.
 *
 * - 어느 웹 주소에서 온 요청을 받아 줄지 정합니다(허용 목록).
 * - `/api/회원`, `/api/예약`처럼 주소별로 담당 기능을 연결합니다.
 * - 서버에 올라간 사진·영상 파일을 내려받을 수 있게 열어 둡니다.
 *   단, 교육영상 원본은 직접 내려받지 못하도록 막습니다.
 * - 중간에 오류가 나면 사용자에게 보여 줄 메시지로 정리해 돌려줍니다.
 */
...
export function createApp() {
  ...
  const denyDirectVideoAccess = (req, res) => {
    res.status(403).json({
  ...
  app.use("/uploads/videos", denyDirectVideoAccess);
  app.use("/uploads/academy/videos", denyDirectVideoAccess);
