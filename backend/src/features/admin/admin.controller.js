...
export async function createStudioMember(req, res, next) {
  try {
    const authUser = await requireAdminDashboardAccess(req, res, "member.write");
    if (!authUser) return;
    const member = await adminService.createStudioMember(req.body || {});
...
function normalizePageOverrideValue(type, value) {
  if (String(type || "").trim() !== "text" || typeof value !== "string") {
    return value;
  }
...
export async function savePageOverride(req, res, next) {
  try {
    ...
    const normalizedType = String(type).trim().slice(0, 80);
    const normalizedValue = normalizePageOverrideValue(normalizedType, value);
    await query(
      ...
      [randomUUID(), normalizedType, String(key).slice(0, 599), JSON.stringify(normalizedValue ?? null)]
    );
    res.json({ ok: true });
  } catch (error) { next(error); }
}
