 ...
 * [문자·알림 발송 API]
 *
 * 관리자 화면의 메시지 기능이 호출하는 서버 주소들입니다.
 *
 * - 발송 설정이 갖춰졌는지 알려 줍니다.
 * - 지금 보내기와 예약 보내기를 접수합니다.
 * - 보낸 내역과 자동 알림 내역을 조회합니다.
 *
 * 접수한 메시지는 곧바로 나가지 않고 발송 대기열에 저장됩니다.
 * 자동 알림 스케줄러가 대기열을 꺼내 실제로 발송합니다.
 * 외부 발송 설정이 꺼져 있으면 대기열에 그대로 남아 있다가,
 * 나중에 설정을 켜면 밀려 있던 것부터 발송됩니다.
 *
 * 화면의 "설정 상태"와 "테스트 모드" 표시가 지금 실제로 나가는지를 알려 줍니다.
 * 전화번호처럼 민감한 정보는 저장할 때 암호화하고 화면에는 가려서 보여 줍니다.
 */
