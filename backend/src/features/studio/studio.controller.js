...
import { resolveSessionUser, isAdminUser as isAdmin } from "../../shared/middlewares/auth.js";
...
export async function listAllBookings(req, res, next) {
  try {
    ...
    const bookings = await studioService.listAllBookingsForAdmin({ from, to, status, branchId });
    res.json({ bookings });
  } catch (error) {
    next(error);
  }
...
export async function verifySalesPinHandler(req, res, next) {
  try {
    ...
    res.json(await studioService.verifySalesPin(req.body?.pin));
  } catch (error) { next(error); }
}
