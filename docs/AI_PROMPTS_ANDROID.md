...
| 3 | G. 이미지·자산 최적화 | ✅ 완료 (2026-08-20, 커밋 4b6a73f) — AAB 44.59 → 11.30MB | ❌ | ❌ |
| 4 | E. 실기기 QA (앱 셸) | ✅ 완료 (2026-08-20, 에뮬레이터) — 접근성 1건 | ❌ | ❌ |
| 5 | E2. 핵심 기능 검증 (백엔드 연결) | ✅ 완료 (2026-08-20) — 영상 재생 버그 발견·수정 (커밋 6693944) | ❌ | ❌ |
| 6 | J. 접근성 보강 | ✅ 완료 (2026-08-20, 커밋 80c349c) — aria-label 60→81, 터치 타겟·포커스 보강 | ❌ | ❌ |
| 7 | I. 스토어 등록 자산 제작 | ✅ 완료 (2026-08-20, 커밋 8446ab8) — 스크린샷 5장·문안 초안. 아이콘·피처그래픽은 디자인 대기 | ❌ | ❌ |
| **8** | **F. 서명 및 업로드 준비** | **⬅ 다음 실행** — 계정 발급 후 | ✅ | ✅ |

**코드 작업은 완료됐다.** 이후 단계는 Google Play 개발자 계정 발급, keystore 생성,
브랜드 아이콘·피처그래픽 디자인, 스토어 문안 검수, 운영 데이터 준비를 기다린다.
H2/H3 hotfix 배포와 H1 운영 조회도 사용자 실행 대기 상태다.
...
# 프롬프트 E2. 핵심 기능 검증 (백엔드 연결)

프롬프트 E 는 백엔드 미연결 상태의 앱 셸만 검증했다. 앱의 핵심 기능이 아직 미검증이다.
...
AGENTS.md, docs/PROJECT_RULES.md, docs/audits/android-device-qa-20260820.md 를 읽어라.

[배경]
프롬프트 E 는 백엔드 미연결로 앱 셸 동작만 검증했다.
이 앱은 리더 앱 전략이라 앱에서 하는 일이 "보유 영상 시청 + 수업 예약" 인데
그 둘이 모두 미검증 상태다. 출시 전 반드시 확인해야 한다.

[환경]
개발 서버 / 개발 DB(homepage_dev) 또는 homepage_test 기준.
운영 서버·운영 DB·운영 키 사용 금지.
필요하면 android-reverse / 개발 터널 스크립트를 사용해라.

[검증 대상]

A. 영상 재생 — 앱의 핵심 기능
   - 보유 영상이 있는 계정으로 앱에서 로그인
   - 영상 목록에 보유 영상이 표시되는지
   - 재생 시작, 일시정지, 탐색(seek)
   - 전체화면 전환과 복귀
   - 백그라운드 전환 시 재생 중지 여부
   - 진도 저장 — 재생 중단 후 재진입 시 이어보기
   - 미보유 영상의 직접 재생 차단 (URL·API 직접 호출 포함)
   - 재생 토큰(ACADEMY_PLAYBACK_TOKEN) 만료 동작

B. 수업 예약
   - 예약 캘린더 로딩과 슬롯 표시
   - 예약 생성 → 서버 반영 → 화면 반영
   - 예약 취소
   - 정원 초과 / 수강권 부족 상태의 버튼 표시
   - 앱 재시작 후 예약 상태 유지

C. 결제 경로 — 현황 확인만
   - 프롬프트 C 에서 결제 진입점이 CartPage 하나뿐이고
     교육영상은 앱에서 차단, 스튜디오 수강권 결제는 미구현으로 확인됐다.
   - 앱에서 실제로 결제 가능한 경로가 하나라도 있는지 확인해라.
   - 없다면 그 사실을 명확히 보고해라. 실제 결제를 시도하지 마라.

D. 푸시 알림
   - google-services.json 이 없어 푸시가 비활성 상태다.
   - 앱에서 알림 권한 요청 UI 가 뜨는지, 뜬다면 거부해도 앱이 정상 동작하는지
   - 푸시 미설정 상태에서 오류나 크래시가 없는지

[출력]
| 영역 | 항목 | 결과 | 재현 조건 | 심각도 | 근거 |
결과는 정상 / 문제 / 미확인 중 하나.

[제약]
- 코드를 수정하지 마라. 검증 결과만 보고해라.
- 실제 결제·SMS·알림톡 발송을 하지 마라. TEST_SAFE_MODE 유지.
- 운영 환경 접근 금지.
- 확인하지 못한 항목은 "미확인"으로 명시하고 무엇이 필요한지 적어라.
- docs/AI_PROMPTS_ANDROID.md 의 공통 제약을 따라라.

[결과 저장]
docs/audits/android-core-qa-YYYYMMDD.md
```
...
프롬프트 D·E 에서 공통 지적된 항목. Play Console pre-launch report 경고를 예방한다.
...
[배경]
프롬프트 E 실측: 소비자 페이지 버튼 214개 중 aria-label 47개.
아이콘 전용 버튼 다수에 접근성 이름이 없다.
Play Console 첫 업로드 시 pre-launch report 가 자동 실행되며,
접근성 경고가 심사 지연으로 이어진 국내 사례가 있다.

[범위]
소비자(비로그인·일반회원) 화면을 우선한다.
관리자 화면은 pre-launch report 크롤러가 도달하지 못하므로 후순위다.
범위를 넓히지 마라.

[작업]

1. 아이콘 전용 버튼 인벤토리
   - 텍스트 없이 아이콘만 있는 button / a 요소를 소비자 화면에서 전부 찾아라.
   - 각각의 실제 기능을 파악해 적절한 한국어 aria-label 을 제안해라.
   - 파일:라인과 제안 라벨을 표로 먼저 보고하고 승인받아라.

2. 승인 후 적용
   - aria-label 추가. 시각적 변화가 없어야 한다.
   - img 태그의 alt 누락도 함께 채워라. 장식용 이미지는 alt="" 로 명시해라.
   - form input 의 label 연결(htmlFor/id)이 빠진 곳을 확인하고 보강해라.

3. 터치 타겟
   - 48x48dp 미만인 클릭 요소를 찾아라.
   - 레이아웃이 바뀌는 변경이므로 목록만 보고하고 적용은 별도 승인을 받아라.
   - 시각적 크기를 키우지 않고 터치 영역만 넓히는 방법(패딩, ::after 확장)을 우선 검토해라.

4. 포커스
   - outline: none 이 적용된 곳을 찾아 키보드 포커스가 보이지 않는지 확인해라.
   - 있다면 대체 포커스 스타일을 제안해라.

[제약]
- 시각 디자인을 바꾸지 마라. 접근성 속성 추가가 원칙이다.
- 관리자 화면으로 범위를 확장하지 마라.
- 기능 로직을 건드리지 마라.
- 적용 후 npm run build 가 통과하는지 확인해라.
- docs/AI_PROMPTS_ANDROID.md 의 공통 제약을 따라라.

[결과 저장]
docs/audits/accessibility-YYYYMMDD.md
보강 전후 aria-label 개수를 수치로 비교해 기록해라.
```
