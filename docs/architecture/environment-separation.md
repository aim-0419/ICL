...
`homepage_dev`는 두 대의 컴퓨터와 개발 서버가 함께 쓴다. 기능을 만들다 보면 테스트 데이터가 뒤엉키는데, 그때 되돌릴 수 있어야 공유 DB를 유지하면서 마음껏 작업할 수 있다.
...
스냅샷은 저장소 바깥의 `icl-dev-snapshots/`에 시각이 붙은 파일로 저장된다. 개발용이라도 실제 회원 행이 들어 있으므로 저장소 안에 두지 않으며, 스크립트가 저장소 내부 경로를 거부한다. 읽기 전용 스냅샷 트랜잭션 안에서 읽으므로 개발 서버가 동시에 쓰고 있어도 일관된 한 시점을 담는다.

되돌릴 때는 파일을 지정하고 명시적으로 승인한다.

```bash
cd backend
DEV_SNAPSHOT_PATH=../../icl-dev-snapshots/homepage_dev_....json.gz ALLOW_DEV_SNAPSHOT_RESTORE=true npm run db:restore:dev
```

되돌리면 개발 DB의 모든 행이 스냅샷 상태로 바뀐다. 다른 컴퓨터와 개발 서버가 보는 데이터도 함께 바뀌므로 승인 플래그를 따로 요구한다.

스냅샷을 뜬 뒤에 컬럼이 사라졌거나 테이블이 없어졌다면 넣을 자리가 없으므로 DB를 손대기 전에 중단한다. 반대로 스냅샷 이후에 추가된 테이블·컬럼은 복원을 막지 않고 결과에 함께 보고한다.

## 개발 기준 데이터

두 컴퓨터가 함께 쓸 기준 데이터는 DB dump로 주고받지 않고 시드 스크립트로 관리한다. 스크립트는 Git에 남아 버전 관리와 리뷰가 되고, 언제든 같은 결과로 다시 만들 수 있으며, 여러 사람이 고쳐도 병합된다. dump 파일은 셋 중 어느 것도 되지 않는다.

```bash
cd backend
npm run db:seed:dev:accounts
```

로컬 DB에서 만든 데이터를 공유 개발 DB로 올리는 방향은 사용하지 않는다. 데이터에는 병합이 없어서 두 컴퓨터가 각각 만든 행을 합칠 수 없고, 한쪽 상태로 덮으면 다른 쪽 작업과 개발 서버의 상태가 사라진다.
