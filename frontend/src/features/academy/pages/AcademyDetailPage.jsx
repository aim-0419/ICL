...
const DETAIL_SUMMARY = {
  입문: {
    ...
    lessons: "총 12강",
    summary:
      "기초 체형 이해부터 수업 도입 멘트까지 단계적으로 익히는 입문 과정입니다. 실제 현장에서 바로 활용할 수 있는 문장과 동작 흐름 위주로 구성됩니다.",
    curriculum: [
      "기본 정렬 체크 포인트",
      "호흡 큐잉 적용법",
      "첫 수업 진행 스크립트",
  ...
  초급: {
    ...
    lessons: "총 20강",
    summary:
      "큐잉 언어를 고도화하고 시퀀스를 설계하는 초·중급 연계 과정입니다. 체형별 보완 전략과 수정 지시 문장을 집중적으로 다룹니다.",
    curriculum: [
      "리포머/체어 핵심 큐잉",
      "시퀀스 설계 구조",
      "체형별 교정 플로우",
...
export function AcademyDetailPage() {
  ...
  useEffect(() => {
    if (!videoId) return;
    listAcademyReviews(videoId).then(setReviews).catch(() => {});
  }, [videoId]);
  ...
  async function handleReviewSubmit(e) {
    ...
    setReviewError("");
    try {
      await createAcademyReview(videoId, reviewForm);
      setReviewForm({ rating: 5, content: "" });
      const updated = await listAcademyReviews(videoId);
      setReviews(updated);
    } catch (err) {
  ...
  async function handleDeleteReview(reviewId) {
    ...
    try {
      await deleteAcademyReview(reviewId);
  ...
  const totalDuration = calcDetailTotalDuration(chapters);
      ...
      <main className="content-page academy-detail-page">
          ...
          <article className="academy-detail-info">
            ...
            <dl className="academy-detail-spec">
              ...
              {avgRating ? (
                <div>
                  <dt>수강 후기</dt>
                  <dd>★ {avgRating} ({reviews.length}건)</dd>
                </div>
              ) : null}
            </dl>
        ...
        <section className="academy-reviews-section">
          ...
          {store.currentUser ? (
            <form className="review-write-form" onSubmit={handleReviewSubmit}>
              <div className="review-form-row">
                ...
                <span className="review-rating-label">{reviewForm.rating}점</span>
              </div>
              <textarea
              ...
              <button type="submit" className="pill-button small" disabled={reviewSubmitting}>
          ...
          <div className="review-list">
            {reviews.length === 0 ? (
              <p className="review-empty">아직 등록된 후기가 없습니다. 첫 번째 후기를 남겨보세요.</p>
            ) : (
              reviews.map((review) => {
