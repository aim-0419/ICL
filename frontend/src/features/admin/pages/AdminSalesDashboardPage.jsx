            ...
            {!loadError && requests.length > 0 ? (
              <div className="refund-request-list">
                {requests.map((request) => {
                  const selectedCount = Array.isArray(request.selectedProductIds) ? request.selectedProductIds.length : 0;
                  return (
                    <article key={request.id} className="refund-request-card">
                      ...
                      <dl className="refund-request-meta">
                        ...
                        <div className="refund-meta-row"><dt>환불 상품</dt><dd>{selectedCount}개</dd></div>
                      </dl>
  ...
  const apexChartOptions = useMemo(() => ({
    ...
    fill: {
      ...
      gradient: {
        ...
        type: "vertical",
        shadeIntensity: 0.4,
        gradientToColors: ["#e8d9c6", "#a07848"],
        inverseColors: false,
        opacityFrom: [0.9, 1],
        opacityTo: [0.55, 0.85],
        stops: [0, 100],
    ...
    markers: {
      size: [0, 6],
      colors: ["transparent", "#fff8f2"],
      strokeColors: ["transparent", "#6f5335"],
      strokeWidth: [0, 2.5],
      hover: { size: 9 },
    },
    xaxis: {
      categories: chartSeries.map((item) => item.label),
      labels: { style: { fontSize: "11px", colors: "#9a8070" } },
    ...
    yaxis: {
      labels: {
        ...
        style: { fontSize: "11px", colors: "#9a8070" },
      },
    },
    tooltip: {
      shared: true,
    ...
    legend: {
      ...
      labels: { colors: "#5a4a3a" },
      itemMargin: { horizontal: 12 },
    },
    grid: {
      borderColor: "rgba(98,73,35,0.08)",
      strokeDashArray: 5,
  ...
  const apexChartSeries = useMemo(() => [
    {
      name: "총매출",
      type: "bar",
      data: chartSeries.map((item) => toAmount(item.grossRevenue || item.totalRevenue)),
    },
    {
      name: "순매출",
      type: "line",
      data: chartSeries.map((item) => toAmount(item.netRevenue)),
    },
  ], [chartSeries]);
    ...
    <div className="admin-sales-report-shell">
      <aside className="admin-sales-sidebar" aria-label="매출 관리자 메뉴">
        ...
        <nav className="admin-sales-sidebar-nav">
          ...
          <div className="admin-sales-sidebar-group active">
            <Link to="/admin/sales">매출 관리</Link>
            <Link className="active" to="/admin/sales">매출 대시보드</Link>
            <a href="#sales-detail">매출 상세 내역</a>
            <button
              type="button"
          ...
          <Link to="/admin/settings">설정 관리</Link>
        </nav>
        ...
        <section className="admin-sales-report-head">
          ...
          <div className="admin-sales-report-controls">
            <div className="admin-sales-period-tabs" aria-label="매출 조회 단위">
              {SALES_PERIOD_OPTIONS.map((option) => (
                <button
                  ...
                  className={`admin-sales-period-tab${
                    period === option.value && !resolvedRange.isCustomRange ? " active" : ""
                  }`}
                  aria-pressed={period === option.value && !resolvedRange.isCustomRange}
                  onClick={() => {
                    setErrorMessage("");
                    setPeriod(option.value);
                    if (option.value === "day") {
                      setDateRange({ startDate: defaultEndDate, endDate: defaultEndDate });
                      setAppliedDateRange({ startDate: defaultEndDate, endDate: defaultEndDate });
                      return;
                    }
              ...
              <button
                type="button"
                className={`admin-sales-period-tab${resolvedRange.isCustomRange ? " active" : ""}`}
                onClick={() => {
