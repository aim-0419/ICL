...
export async function listAdminPassTransactions({ limit = 200, date = "" } = {}) {
  const params = new URLSearchParams({ limit: String(limit) });
  if (date) params.set("date", date);
  const result = await apiRequest(`/studio/admin/pass-transactions?${params}`);
  return Array.isArray(result?.transactions) ? result.transactions : [];
}

export async function listAdminConsultations({ date = "", staffName = "", type = "", search = "" } = {}) {
  ...
  const result = await apiRequest(`/studio/admin/consultations${qs ? `?${qs}` : ""}`);
  return Array.isArray(result?.consultations) ? result.consultations : [];
}

export async function createAdminConsultation(payload) {
  const result = await apiRequest("/studio/admin/consultations", { method: "POST", body: payload });
  return result?.consultation || result;
}

export async function deleteAdminConsultation(id) {
  return apiRequest(`/studio/admin/consultations/${encodeURIComponent(String(id))}`, { method: "DELETE" });
}
...
export async function listAdminAllBookings({ from, to, status, branchId, classStatus } = {}) {
  const query = new URLSearchParams();
  if (from) query.set("from", from);
  if (to) query.set("to", to);
  if (status) query.set("status", status);
  if (branchId) query.set("branchId", branchId);
  if (classStatus) query.set("classStatus", classStatus);
  const suffix = query.toString() ? `?${query.toString()}` : "";
