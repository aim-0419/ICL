...
export function StudioReservationPage() {
  ...
  const hasActivePass = ticketItems.some((ticket) => !ticket.expired);
    ...
    return (
      <PageLayout mainClass="dashboard-page">
        <section className="dashboard-hero mypage-hero-card">
          ...
          <h1>수강권 정보를 불러오는 중입니다</h1>
          <p className="studio-gate-loading-msg">잠시만 기다려 주세요...</p>
        </section>
      </PageLayout>
    );
  }

  if (dataLoaded && !hasActivePass) {
    return (
      <PageLayout mainClass="dashboard-page">
        <section className="dashboard-hero mypage-hero-card">
          <p className="section-kicker">필라테스 예약하기</p>
          <h1>수업 예약과 수강권을 확인합니다</h1>
        </section>
        <section className="dashboard-card studio-no-pass-card">
          <div className="studio-no-pass-inner">
            <div className="studio-no-pass-icon">
              <svg width="64" height="64" viewBox="0 0 64 64" fill="none">
                <circle cx="32" cy="32" r="32" fill="#f3f4f6"/>
                <rect x="14" y="26" width="36" height="22" rx="4" fill="#d1d5db"/>
                <rect x="14" y="26" width="36" height="8" rx="4" fill="#9ca3af"/>
                <circle cx="32" cy="18" r="8" fill="none" stroke="#9ca3af" strokeWidth="3"/>
                <path d="M24 18 C24 13.6 27.6 10 32 10 S40 13.6 40 18" stroke="#9ca3af" strokeWidth="3" fill="none" strokeLinecap="round"/>
              </svg>
            </div>
            <h2>현재 보유하고 있는 수강권이 없습니다</h2>
            <p>수강권을 등록하면 이끌림 필라테스 수업을 예약하고<br/>일정을 확인할 수 있습니다.</p>
            <p className="studio-no-pass-sub">수강권 등록 문의는 센터로 연락해 주세요.</p>
            <div className="studio-no-pass-contacts">
              <span>장덕점</span>
              <span>효천점</span>
            </div>
            <button
