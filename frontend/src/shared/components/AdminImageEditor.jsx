...
function normalizeOverrideEntries(type, overrides) {
  if (!overrides || typeof overrides !== "object") return {};
  if (type !== "image") return overrides;
  return Object.fromEntries(
    Object.entries(overrides).map(([key, value]) => [key, normalizePageOverrideAsset(value)])
  );
}
...
function getRichTextPlainText(value) {
  ...
  const safeHtml = sanitizeRichTextHtml(text);
  if (typeof document === "undefined") return safeHtml.replace(/<[^>]*>/g, "").trim();
...
function textToEditorHtml(value) {
  if (isHtmlTextValue(value)) return sanitizeRichTextHtml(value);
...
function normalizeEditorHtmlForTarget(html, target) {
  const raw = sanitizeRichTextHtml(html).trim();
  if (!raw) return "";
...
export function AdminImageEditor() {
  ...
  const [imageOverrides, setImageOverrides] = useState(() => ({
    ...DEFAULT_PAGE_IMAGE_OVERRIDES,
    ...normalizeOverrideEntries("image", readOverrides(IMAGE_STORAGE_KEY)),
  }));
  ...
  useEffect(() => {
    ...
    fetchOverridesFromDb().then((data) => {
      ...
      const mergeAndApply = (storageKey, type, setter, ref) => {
        const defaults = type === "image" ? DEFAULT_PAGE_IMAGE_OVERRIDES : {};
        const local = normalizeOverrideEntries(type, readOverrides(storageKey));
        const remote = normalizeOverrideEntries(type, grouped[type] || {});
        const merged = { ...defaults, ...local, ...remote };
        if (type === "text") {
          Object.keys(merged).forEach((key) => {
            if (typeof merged[key] === "string" && isHtmlTextValue(merged[key])) {
              merged[key] = sanitizeRichTextHtml(merged[key]);
            }
          });
        }
  ...
  const finishInlineTextEditing = useCallback(
    (save = true, htmlValue = "") => {
      ...
      if (hasExplicitHtml) {
        ...
        const normalizedValue = sanitizeRichTextHtml(htmlValue).trim();
